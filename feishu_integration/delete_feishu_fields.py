...
class FeishuFieldDeleter:
    ...
    def delete_all_fields(self, auto_confirm=False):
        """删除所有字段（保留主字段）"""
        print("🚀 开始删除飞书多维表格字段...")
        ...
        # 过滤掉主字段（主字段无法删除）
        deletable_fields = [f for f in fields if not f.get("is_primary", False)]

        if not deletable_fields:
            print("✅ 没有可删除的字段（只有主字段无法删除）")
            return

        print(f"\n⚠️  即将删除 {len(deletable_fields)} 个字段（保留主字段）")

        if auto_confirm:
            print("🤖 自动确认模式：直接删除所有可删除字段")
        else:
            try:
                confirm = input("确认要删除所有这些字段吗？(输入 'yes' 确认): ")
        ...
        for i, field in enumerate(deletable_fields, 1):
            field_id = field.get("field_id")
            field_name = field.get("field_name")

            print(f"   [{i}/{len(deletable_fields)}] 正在删除字段: {field_name}")
        ...
        print(f"   ⚠️  保留主字段: {len(fields) - len(deletable_fields)} 个")

def main():
    import sys
    auto_confirm = "--auto-confirm" in sys.argv

    deleter = FeishuFieldDeleter()
    deleter.delete_all_fields(auto_confirm=auto_confirm)
